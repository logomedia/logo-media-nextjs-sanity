...
export async function generateMetadata({ params }) {
  ...
  const ogImage = pageData.ogImage
    ? urlFor(pageData.ogImage.asset).width(800).url()
    : "";
...
export default async function Page({ params }) {
  ...
  const [pageData, reviews, projects, partners, posts] = await Promise.all([
    pageRequest,
    reviewsData,
    projectsData,
    partnersData,
      ...
      <>
        ...
        {content && !isEnabled ? (
          ...
          <PreviewSuspense fallback={<LoadingPreview />}>
            <PreviewRenderSections service={page} preview={isEnabled} />
          </PreviewSuspense>
        )}
