...
export default function BestForCustomers(props) {
	const { heading, description1, description2, eyebrow, sectionHeading, sectionDescription, stats, image, imageAltText } = props
	...
	return (
		...
		>
			<Typography paragraph variant='overline' sx={{ color: "primary.main", textAlign: { xs: "center", md: "left" } }}>
				{eyebrow}
			</Typography>
			...
			<Section sectionHeading={sectionHeading} sectionDescription={sectionDescription} stats={stats} image={image} imageAltText={imageAltText} />
		</Container>
	)
}
...
function Section({ sectionHeading, sectionDescription, stats, image, imageAltText }) {
	return (
		<StyledSection>
			<Stack
				sx={{
			...
			>
				...
				>
					...
					<StyledPortableText value={sectionDescription} variant='' sx={{ opacity: 0.72 }} />
				</Stack>
			...
			<Box sx={{ position: "absolute", width: 1, height: 1, top: 0 }}>
				<Image alt={imageAltText} src={urlFor(image.asset).url()} sx={{ width: 1, height: 1 }} />
			</Box>
		</StyledSection>
	)
}
