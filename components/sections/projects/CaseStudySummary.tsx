    ...
    <RootStyle>
      <Stack spacing={3}>
        <Stack spacing={2} sx={{position:'relative'}}>
          ...
          <Box>
          <Image style={{width:'100%', height:'100%'}} src={logo.asset.url} alt={name} width={logo.asset.metadata.dimensions.width} height={logo.asset.metadata.dimensions.height} />
          </Box>
        ...
        <Stack spacing={1}>
          ...
          {tags.map((tag) => (
            <Typography key={tag} variant="body2" sx={{ pb: 1 }}>{tag}</Typography>
                ))}
